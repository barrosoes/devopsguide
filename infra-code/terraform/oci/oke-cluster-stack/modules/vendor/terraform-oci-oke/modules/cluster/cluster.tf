...
resource "oci_containerengine_cluster" "k8s_cluster" {
  ...
  lifecycle {
    # Importante: não ignorar defined/freeform_tags para que mudanças em tags
    # apareçam no plan/apply (por padrão o módulo upstream ignora).
    ignore_changes = [cluster_pod_network_options, options[0].kubernetes_network_config]
